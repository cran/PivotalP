#' @export
nmixp<-function(data,s,n,a,parameters,conf=0.95){
  ...
  l=x[r]
  m=x[s]
  pmix=a*(pnorm(x[r],l1,b1 ))+(1-a)*(pnorm(x[r],l2,b2 ))
  p=1-((1-pmix)^(u+1))
  fun=function(t){
    a*pnorm(qnorm(t,l2,b2 ),l1,b1 )+(1-a) * t - p
  }
  ...
  return(structure(int, class = "nmixp"))

}
#' @export
print.nmixp <- function(x, ...) {
  if (!inherits(x, "nmixp"))
    stop("Use only with 'nmixp' objects")
  cat("Prediction  for the future observation based on mixture of two normal distribution \n")
  cat("Point:\n")
